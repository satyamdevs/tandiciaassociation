...
function UpcomingCampsPanel() {
  ...
  const handleBannerUpload = async (file) => {
    ...
    try {
      ...
      const fileName = `upcoming-camps/${timestamp}_${file.name}`;
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.name.trim() || !formData.camp_date) {
      alert("Please enter camp name and date");
      return;
    }
    ...
    try {
      ...
      if (formData.banner_url) {
        campData.banner_url = formData.banner_url;
      }
      ...
      setFormData({ name: "", camp_date: "", location: "", time: "", description: "", banner_url: "" });
      setShowForm(false);
      setEditingCamp(null);
      fetchCamps();
    } catch (error) {
      console.error("Save error:", error);
      alert(`Failed to save camp: ${error.message}`);
    }
  };
  ...
  const handleDelete = async (id) => {
    if (!confirm("Delete this upcoming camp?")) return;
      ...
      <section ref={heroRef} className="relative px-6 md:px-12 py-12 md:py-16 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto">
          ...
          >
            ...
            {camps.length} {camps.length === 1 ? "Camp" : "Camps"} Scheduled
          </motion.div>
          ...
          >
            Schedule and manage future eye care camps.
          </motion.p>
              ...
              <div className="bg-white border border-gray-100 rounded-3xl p-8 shadow-sm mb-12">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div className="grid md:grid-cols-2 gap-5">
                    <div>
                      <label className="block text-sm font-medium text-gray-600 mb-2">Camp Name *</label>
                      <input
                        type="text"
                        value={formData.name}
                        onChange={(e) => setFormData(prev => ({ ...prev, name: e.target.value }))}
                        className="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition"
                        placeholder="Vision Care Camp"
                        required
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setFormData(prev => ({ ...prev, camp_date: e.target.value }))}
                        className="w-full px-4 py-3.5 bg-gray-50 border border-gray-200 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition"
                        required
                      />
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4">
                      {formData.banner_url && (
                        <div className="w-32 h-20 bg-gray-50 rounded-xl overflow-hidden">
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {camps.map((camp, idx) => (
                ...
                >
                  ...
                  <h3 className="text-lg font-serif text-gray-900 mb-3">{camp.name}</h3>
                  ...
                  {camp.description && (
                    <p className="text-gray-500 text-sm line-clamp-3">{camp.description}</p>
                  )}
                </motion.div>
              ))}
            </div>
...
function AdminPanel({ activeTab, setActiveTab }) {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* Tab Content */}
      {activeTab === "engagements" ? <EngagementsPanel /> : 
